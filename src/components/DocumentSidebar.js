...
const DocumentSidebar = ({
  ...
  const child = data.filter(
    (filtredelement) => filtredelement.parentNumber === row.numberRow
  );
  return (
    <>
      {child.length > 0 ? (
        ...
        >
          {row.idDirectoryTypeRow >= 5 && row.idDirectoryTypeRow <= 6 ? (
            <div style={{ display: "flex", width: "100%" }}>
              ...
              <ListItemButton onClick={() => dispatch(setShowElement(row))}>
                <ExpandLess style={{ opacity: "0" }} />
                <ListItemText
                  primary={
                    row.directoryNameRow +
